            ...
            <section className="py-6 space-y-4 relative">
                <div className="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p className="mt-1 text-2xs text-neutral-500 leading-relaxed">Bu formun bağlı olduğu hedefi görüntüleyebilirsiniz.</p>
                    </div>
                </div>
                <div className="rounded-xl border border-white/10 bg-neutral-900/40 px-4 py-3 text-xs text-neutral-200">
                    {hasLinkedForm ? (
                        <div className="space-y-1">
                            <p className="text-2xs text-neutral-500 uppercase tracking-[0.18em]">Bağlı Form</p>
                            <p className="text-sm font-semibold text-neutral-50">{linkedFormTitle || linkedFormId}</p>
                        </div>
                    ) : (
                        <div className="space-y-1">
                            <p className="text-2xs text-neutral-500 uppercase tracking-[0.18em]">Bağlı değil</p>
        ...
        <section className="py-6 space-y-4 relative">
            ...
            <div className={`flex items-start justify-between gap-4 transition-opacity duration-300 ${allowAnonymousResponses ? "opacity-20" : ""}`}>
                <div>
                    ...
                    <p className="mt-1 text-2xs text-neutral-500 leading-relaxed">Bir sonraki formun ne zaman açılacağını kontrol etmek için bağımlılık kurun.</p>
                </div>
                <span className="rounded-full border border-white/10 bg-white/5 px-3 py-0.5 text-3xs uppercase tracking-[0.18em] text-neutral-400">Opsiyonel</span>
            </div>

            <div className={`space-y-3 transition-opacity duration-300 ${allowAnonymousResponses ? "opacity-20" : ""}`}>
                <label className="px-0.5 text-2xs font-semibold uppercase tracking-[0.18em] text-neutral-400">Hedef form</label>
                <div className="relative" ref={formPickerRef}>
                    <div className="relative">
                        ...
                        <button type="button" aria-haspopup="dialog" aria-expanded={showFormPicker} onClick={() => !allowAnonymousResponses && setShowFormPicker((prev) => !prev)} className="flex w-full items-center justify-between rounded-lg border border-white/10 bg-neutral-900/60 pl-9 pr-3 py-2 text-left text-sm text-neutral-100 shadow-sm outline-none transition hover:bg-white/5 focus:border-skylab-400/50 focus:ring-2 focus:ring-skylab-400/40">
                            ...
                            <SearchPicker searchValue={formSearch} onSearchChange={setFormSearch} autoFocus items={filteredForms} itemsPerPage={4} activeItemId={linkedFormId || undefined} getItemId={(form) => form.id}
                                ...
                                renderItem={(form, { active, onSelect }) => (
                                    <button type="button" onClick={onSelect} className={`flex w-full items-start gap-2 px-3 py-2 text-left text-sm transition hover:bg-white/10 ${active ? "bg-white/15 text-neutral-100 ring-1 ring-white/20" : "text-neutral-200"}`}>
                                        <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-skylab-400/70" />
                                        <div className="flex-1"><p className="font-medium leading-tight">{form.title}</p><p className="text-2xs text-neutral-500">{form.id}</p></div>
                                    </button>
                                )}
                            />
                        ) : null}
                ...
                <AnimatePresence>
                    {hasLinkedForm && (
                        <motion.div key={"linked-form-alert"} variants={variants} initial="hidden" animate="visible" exit="exit" transition={{ duration: 0.2, ease: [0.22, 1, 0.36, 1] }}>
                            <div className="flex items-center justify-between gap-3 rounded-xl border border-skylab-400/40 bg-skylab-500/10 px-4 py-3 text-xs text-skylab-300">
                                <span className="truncate">{linkedFormId} ile eşleştirildi.</span>
                                <button type="button" className="text-skylab-300/80 transition-colors hover:text-skylab-300" onClick={() => handleRequestLink("")}>Kaldır</button>
                            </div>
                        </motion.div>
                    )}
                </AnimatePresence>
            ...
            <div className={`absolute inset-0 flex items-center justify-center transition-opacity duration-300 ${allowAnonymousResponses ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
                <span className="px-3 py-1.5 rounded-full border border-white/10 bg-neutral-900/80 text-2xs font-medium text-neutral-200">Anonim cevap açık olduğundan kapalıdır</span>
            </div>
        </section>
    );
}