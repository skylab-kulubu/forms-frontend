...
import { Trash2, ChevronsLeft, Lock } from "lucide-react";
import { DrawerTrigger } from "../../utils/Drawer";

export const LibraryTrigger = forwardRef(({ dragSource, isDropOver, isLgUp, isLockedDrag = false }, ref) => {
    const isDeleteTarget = isDropOver && !isLockedDrag;
                ...
                <motion.button type="button" layout initial={false} animate={dragSource === "canvas" ? "dragging" : "idle"}
                    variants={{
                        ...
                        dragging: { width: "100%", marginRight: "0rem", borderTopLeftRadius: "9999px", borderBottomLeftRadius: "9999px",
                            borderTopRightRadius: "9999px", borderBottomRightRadius: "9999px", backgroundColor: isDeleteTarget ? "rgba(239, 68, 68, 0.1)" : "rgba(10, 10, 10, 0.6)",borderColor: isDeleteTarget ? "rgba(239, 68, 68, 0.7)" : "rgba(64, 64, 64, 0.8)", x: 0
                        }
                    }}
                    ...
                    title={dragSource === "canvas" ? (isLockedDrag ? "Bu soru akış koşulunda kullanılıyor, silinemez" : "Bırak ve sil") : "Paneli aç"}
                >
                    <AnimatePresence mode="popLayout">
                        {dragSource === "canvas" ? (
                            <motion.div key={isLockedDrag ? "lock-icon" : "trash-icon"} className="flex flex-col items-center justify-center gap-2"
                                initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.5 }} transition={{ duration: 0.2 }}
                            >
                                {isLockedDrag
                                    ? <Lock size={18} className="text-neutral-400" />
                                    : <Trash2 size={20} className={isDeleteTarget ? "text-red-400" : "text-neutral-400"} />}
                                <span className={`text-3xs font-semibold tracking-wide ${isDeleteTarget ? "text-red-300" : "text-neutral-400"}`}>
                                    {isLockedDrag ? "KİLİTLİ" : "SİL"}
                                </span>
                            </motion.div>
                        ) : (
