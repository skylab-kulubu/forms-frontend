...
export function Dropdown({ value, onChange, options = [], placeholder = "Seçiniz...", className = "", renderOption }) {
    ...
    return (
        <div className={`relative ${className}`} ref={containerRef}>
            ...
            <AnimatePresence>
                {isOpen && (
                    <Floating anchor={containerRef} onDismiss={() => setIsOpen(false)} offset={4} matchWidth>
                        <motion.div initial={{ opacity: 0, y: 4, scale: 0.98 }}
                            animate={{ opacity: 1, y: 0, scale: 1 }}
                            exit={{ opacity: 0, y: 4, scale: 0.98 }}
                            transition={{ duration: 0.1 }}
                            className="w-full rounded-lg border border-white/10 bg-[#1a1a1a] shadow-xl backdrop-blur-xl scrollbar"
                        >
